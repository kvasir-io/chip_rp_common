    ...
    private:
        ...
        static std::size_t readBuffer(std::span<std::byte> dest) {
            ...
            bool const    available
              = !static_cast<bool>(bufferState[BufferControlReg<Buffer>::available]);
            ...
            if(transferLength != dest.size()) {
                ...
                transferLength = std::min<std::uint16_t>(transferLength,
                                                         static_cast<std::uint16_t>(dest.size()));
            }
    ...
    public:
        ...
        static bool tryTransfer(TransferType const& data) {
            ...
            if(!buffersAvailable[0] && !buffersAvailable[1]) {
                UC_LOG_W("EP{} {}: Both buffers busy, cannot transfer",
                         EP,
                         IsIn ? "IN"sv : "OUT"sv);
                return false;
            }
