...
namespace Kvasir { namespace USB {
    namespace detail {
        ...
        private:
            ...
            using SofFunctionType
              = std::conditional_t<Config::UseSof,
                ...
                if(ep_num == 0 && in) {
                    ...
                    if(!remainingConfigDescriptor.empty()) {
                        ...
                        if(isLast) {
                            ...
                            remainingConfigDescriptor
                              = remainingConfigDescriptor.subspan(chunkSize);
                        }
                        return true;
                    }
            ...
            static void handleBufferStatus() {
                ...
                while(buffers) {
                    auto const endpointBitIndex
                      = static_cast<std::uint32_t>(std::countr_zero(buffers));
        ...
        public:
            ...
            static bool ep0OUTGetData(std::span<std::byte> data) {
                if(ep0_ctrl.stage() == ControlStage::Data) {
                    ...
                    if(len != data.size()) {
                        UC_LOG_E("USB: Invalid out data (received={}, expected={})",
                                 len,
                                 data.size());
                        return false;
                    }
